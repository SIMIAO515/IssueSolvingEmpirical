...
class AgentController:
    ...
    def __init__(
        ...
        # step hooks for external callbacks
    ...
    def add_step_hook(self, hook: Callable[[int], None]) -> None:
        """Add a callback that will be executed at the beginning of each step.
        
        Args:
            hook: A callable that takes the current iteration number as parameter
        """
        self._step_hooks.append(hook)
    
    def remove_step_hook(self, hook: Callable[[int], None]) -> None:
        """Remove a previously added step hook.
    ...
    def _execute_step_hooks(self) -> None:
        """Execute all registered step hooks with the current iteration number."""
        current_iteration = self.state.iteration_flag.current_value
        for hook in self._step_hooks:
            try:
                hook(current_iteration)
            except Exception as e:
                logger.warning(f'Error executing step hook: {e}')
    ...
    async def _step(self) -> None:
        ...
        # Execute step hooks before proceeding with the step
        self._execute_step_hooks()
